...
TASK0_ANCHOR_WEIGHT = 0.0   # DIAGNOSTIC TEST: anchor loss disabled entirely, on seed=3 specifically
...
def train(str_train_model,tasks,task_class_ids,task_id,feature_list,threshold,X_val,y_val,bool_reorganize_memory,owl_data_labeling=False):
    ...
    for epoch in range(epochs):
        ...
        model.eval() # prep model for evaluation
        # DIAGNOSTIC TEST: Formula 1 disabled, reverted to the ORIGINAL
        # pooled-validation PR-AUC checkpoint metric, isolating Formula 1 as
        # the last remaining suspect for seed=3's collapse (anchor loss
        # already ruled out for this seed).
        val_pred, val_gt = [], []
        for data, target in valid_loader:
            pred = model(data.to(device))[:, 1].reshape(target.shape)
            y_pred = pred.detach().cpu().numpy().tolist()
