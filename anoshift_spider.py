...
#     positive_similarities = torch.cosine_similarity(anchor_representations, positive_representations, dim=-1) / temperature
#     log_positive_similarities = torch.log(positive_similarities + 1e-8)  # Avoid log(0)

#     negative_similarities = torch.cosine_similarity(anchor_representations, negative_representations, dim=-1) / temperature
#     negative_loss = torch.mean(torch.logsumexp(negative_similarities, dim=-1))  # Encourage low similarity with negatives

#     loss = -torch.mean(log_positive_similarities) + negative_loss
#     return loss
...
#     # Pre-compute indices for potential positives and negatives
#     same_class_indices = (batch_labels.unsqueeze(1) == batch_labels).nonzero(as_tuple=False)[:, 1:]  # Exclude self
#     different_class_indices = (batch_labels.unsqueeze(1) != batch_labels).nonzero(as_tuple=False)[:, 1:]

#     # Randomly sample positives and negatives from pre-computed indices
#     positive_indices = torch.randint(0, same_class_indices.shape[1], (batch_size, 1))
#     positive_samples = batch_data[same_class_indices[torch.arange(batch_size), positive_indices]]

#     negative_indices = torch.randint(0, different_class_indices.shape[1], (batch_size, 1))
#     negative_samples = batch_data[different_class_indices[torch.arange(batch_size), negative_indices]]
...
#     global device
#     batch_size = batch_data.shape[0]
#     positive_samples = torch.empty_like(batch_data)
#     negative_samples = torch.empty_like(batch_data)

#     for i in range(batch_size):
#         # Positive: Find a different sample with the same label (excluding self)
#         available_indices = torch.arange(batch_size).to(device)
#         available_indices = available_indices[available_indices != i]
#         same_class_indices = available_indices[batch_labels[available_indices] == batch_labels[i]]
        
#         positive_idx = same_class_indices[torch.randint(len(same_class_indices), (1,))[0]]

#         # Negative: Find a sample with a different label
#         different_class_indices = available_indices[batch_labels[available_indices] != batch_labels[i]]
#         negative_idx = different_class_indices[torch.randint(len(different_class_indices), (1,))[0]]

#         positive_samples[i] = batch_data[positive_idx]
#         negative_samples[i] = batch_data[negative_idx]

#     # print(positive_samples.shape,negative_samples.shape)    

#     return positive_samples.to(device), negative_samples.to(device)
...
def train(str_train_model,tasks,task_class_ids,task_id,feature_list,threshold,X_val,y_val,bool_reorganize_memory):
    ...
    global student_model1,student_model2,student_supervised,student_optimizer1,student_optimizer2,student_supervised_optimizer
    global teacher_model1,teacher_model2,teacher_supervised
    ...
    for epoch in range(epochs):
        ...
        # for epoch in range(epochs):
            ...
            sup_loss = loss_fn(y_pred.float(),F.one_hot(lab_y, 2).float())#to(device)
            ...
            if task_id > 0:
                ...
                if bool_closs:
                    ...
                    anchor_representations = model(lab_X)
        ...
        lr_precision, lr_recall, _ = precision_recall_curve(val_gt, [1-x for x in val_pred], pos_label=0.)
...
def taskwise_lazytrain(train_cutoff=4):
    ...
    train_order = task_order[:train_cutoff]
    test_order = task_order[train_cutoff:]
    ...
    for task_id,task in enumerate(train_order):
        ...
        # feature_list_student2 =train("student2",tasks,task_class_ids,task_id,feature_list_student2,threshold,np.concatenate(val_x_all_tasks, axis=0 ),np.concatenate(val_y_all_tasks, axis=0 ),True)
        feature_list_student1 =train("student1",tasks,task_class_ids,task_id,feature_list_student1,threshold,np.concatenate(val_x_all_tasks, axis=0 ),np.concatenate(val_y_all_tasks, axis=0 ),True)
        
        # feature_list_student_supervised =train("student_supervised",tasks,task_class_ids,task_id,feature_list_student_supervised,threshold,np.concatenate(val_x_all_tasks, axis=0 ),np.concatenate(val_y_all_tasks, axis=0 ),False)
        # teacher_model.load_state_dict(model.state_dict())
    testing(training_cutoff=train_cutoff, seen_data=True) 
    testing(training_cutoff=train_cutoff, seen_data=False)    
...
if __name__ == "__main__":
    ...
    parser.add_argument('--b_m', type=float, default=0.7, metavar='S',help='batch memory ratio(default: 0.2)')
    parser.add_argument('--lr', type=float, default=0.0001, metavar='S',help='batch memory ratio(default: 0.001)')
    parser.add_argument('--wd', type=float, default= 1e-05, metavar='S',help='batch memory ratio(default: 0.01)')
    ...
    parser.add_argument('--bma', type=float, metavar='S',default=0.2,help='batch minority allocation(default: 0.8)')
    ...
    global labels_ratio,no_of_rand_samples,l_rate,w_decay,batch_minority_allocation,b_m,alpha,lab_samp_in_mem_ratio,bool_gpm,mem_strat,temp_filename,auc_result,seed,bool_closs
    b_m = float(args.b_m)
