...
from utils.utils import log,create_directories,trigger_logging,set_seed,get_gpu,load_model,EarlyStopping,GradientRejection,get_dataset_info
...
def load_metadata(dataset_name,lr,w_d,trt):
    # set_seed(125)
    # get_gpu()
...
def load_model_metadata(w_decay):
    ...
    if label in ['cifar10','cifar100','svhn']:
        model = AlexNet()
        temp_model = AlexNet()
        # print("yes i' exiting")
        # exit()
...
def train(tasks,X_val,y_val):
    ...
    for X,y,y_classname in tasks:
        ...
        for epoch in range(epochs):
            ...
            lr_precision, lr_recall, _ = precision_recall_curve(val_gt, [1-val for val in val_pred],pos_label=0)
...
def taskwise_lazytrain():
    ...
    for task_id,task in enumerate(task_order):
        ...
        input_shape,tasks,X_test,y_test,X_val,y_val = load_dataset(pth,task_class_ids,task_minorityclass_ids,tasks_list,task2_list,[task,],bool_encode_benign=False,bool_encode_anomaly=True,label=label,bool_create_tasks_avalanche=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lr', type=float, default=1e-4, metavar='S',help='learning rate(default: 0.001)')
    parser.add_argument('--w_d', type=float, default=1e-5, metavar='S',help='weight decay (default: 0.001)')
    ...
    args = parser.parse_args()
    print("setting seed value",args.seed)
    set_seed(args.seed)
    get_gpu(args.gpu)
    print("seed is",args.seed)
