...
def vime_self(device,x_unlab, p_m, alpha, parameters={'epochs': 5, 'batch_size': 64}):
    # Parameters
    ...
    m_label, x_tilde = pretext_generator(m_unlab, torch.from_numpy(x_unlab))
    ...
    # x_unlab = torch.from_numpy(x_unlab).to(device)
    ...
    for epoch in range(epochs):
        for i in range(0, len(x_unlab), batch_size):
            ...
            loss_feature = criterion_feature(outputs_feature, torch.from_numpy(labels_feature).to(device))
