'''
- Labelling using majority distance using ALL buffer memory samples
- Self-y_classname labelling for unseen classes
- Security analysts gives exactly x correct class 0 samples and y correct class 1 samples
...
def owl_data_labeling_strategy(X, y, y_classname, unseen_task=True):
    ...
    top_class_0_indices = sorted_pred_class_0[1][sorted_pred_class_0[0] > 0.9].detach().cpu()
    ...
    if len(top_class_0_indices) > 1:
        ...
        member_inference_class_0 = np.argsort(cdist(top_class_0_data, memory_X), axis=1)[:,:500]
        maj_labels, count = stats.mode(memory_y[member_inference_class_0],axis=1)
        # member_inference_class_0 = np.argsort(cdist(top_class_0_data, sample_means), axis=1)[:,:5]
        # maj_labels, count = stats.mode(associated_label[member_inference_class_0],axis=1)
    ...
    if len(top_class_1_indices) > 1:
        ...
        start_inference = time.time()
        member_inference_class_1 = np.argsort(cdist(top_class_1_data, memory_X), axis=1)[:,:5]
        maj_labels, count = stats.mode(memory_y[member_inference_class_1],axis=1)
        # member_inference_class_1 = np.argsort(cdist(top_class_1_data, sample_means), axis=1)[:,:5]
        # maj_labels, count = stats.mode(associated_label[member_inference_class_1],axis=1)
...
# def compute_otdd(task_id, X, memory_X, memory_y_name, attack_y_name, benign_y_name):
#     global consecutive_otdd

#     start_otdd = time.time()
#     print('\nComputing otdd distance with previous task...')
#     # creating dataloader for current task data
#     dummy_target_label1 = torch.zeros(X.shape[0], dtype=int)
#     current_task = X
    
#     # creating dataloader for previous task data
#     print(f'Previous task data has labels ({attack_y_name - 2}, {benign_y_name - 2})')
#     prev_data = np.vstack((memory_X[memory_y_name == (attack_y_name - 2)], memory_X[memory_y_name == (benign_y_name - 2)]))
#     dummy_target_label2 = torch.zeros(prev_data.shape[0], dtype=int)

#     otdd_val = compute_ot_distance(current_task, dummy_target_label1, prev_data, dummy_target_label2, device)
#     consecutive_otdd.append(otdd_val)
#     end_otdd = time.time()
#     print(f'Time taken to compute otdd value: {end_otdd - start_otdd} seconds')
...
def train(str_train_model,tasks,task_class_ids,task_id,feature_list,threshold,X_val,y_val,bool_reorganize_memory,owl_data_labeling=False):
    ...
    if owl_data_labeling == False:
        ...
        print(f'Class Imbalance for task {task_id} (% of minority samples)= {CI_list[-1]}\n')
...
def validation_testing():
    ...
    print(f'\nTesting on validation data of first {training_cutoff} tasks')
    ...
    for task_id,task in enumerate(testing_tasks, start = 0):
        ...
        input_shape,tasks,X_test,y_test, X_val, y_val = load_dataset(pth,task_class_ids,task_minorityclass_ids,tasks_list,task2_list,[task,],bool_encode_benign=bool_encode_benign,bool_encode_anomaly=bool_encode_anomaly,label=label,bool_create_tasks_avalanche=False,load_whole_train_data=False)
        
        features,target_label = X_val,y_val
    ...
    if N<2:
        print('Require >=2 seen tasks to compute aut values!')
        exit()
    ...
    auc_result['validation_aut_seen_minority'] = prauc_out_aut
...
def taskwise_lazytrain():
    ...
    truth_agreement_fraction_0 = min(truth_agreement_fraction_0, 0.4)
    truth_agreement_fraction_1 = min(truth_agreement_fraction_1, 0.4)
    print(f'Agreement fraction: class 0 = {truth_agreement_fraction_0}, class 1 = {truth_agreement_fraction_1}')
    avg_CI = np.mean(CI_list)
    print(f'Average CI over training tasks (% of minority samples)= {avg_CI}')
    ...
    test_set_results = []
    with open(temp_filename, 'w') as fp:
        test_set_results.extend([testing(training_cutoff=4, seen_data=True),testing(training_cutoff=4, seen_data=False),str(owl_self_labelled_count_class_0), str(owl_self_labelled_count_class_1),str(owl_analyst_labelled_count_class_0), str(owl_analyst_labelled_count_class_1) ])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lr', type=float, default=0.01, metavar='S',help='learning rate')
    parser.add_argument('--wd', type=float, default= 1e-6, metavar='S',help='weight decay')
    ...
    parser.add_argument('--bool_gpm', type=str, metavar='S',default="False",help='Enables gradient projections(default: True)')
    parser.add_argument('--mem_strat', type=str, metavar='S',default="equal",help='Buffer memory strategy(default: full initialization)')
    parser.add_argument('--training_cutoff', type=int, default=7, metavar='S',help='train the model for first n tasks and test for time decay on the rest')
    ...
    parser.add_argument('--grid_search', type=str, metavar='S',default='False',help='to perform grid search or not')
    ...
    global labels_ratio,no_of_rand_samples,l_rate,w_decay,batch_minority_allocation,b_m,alpha,lab_samp_in_mem_ratio,bool_gpm,mem_strat,temp_filename,auc_result,seed,bool_closs,mlps,training_cutoff, epochs, ds, beta
    global grid_search
    ...
    grid_search = eval(args.grid_search)
