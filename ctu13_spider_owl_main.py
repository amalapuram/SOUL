...
if __name__ == "__main__":
    ...
    parser.add_argument('--wd', type=float, default=1e-2, metavar='S',help='batch memory ratio(default: 0.01)')
    ...
    auc_results = {}
    # seed_list = [1000,224,45454554,25488,5448]
    # seed_list = [0,1,2]
    seed_list = [0,1,3]
    curr_dir = os.getcwd()
    for seed_value in seed_list:
        ...
        proc = subprocess.Popen(["python ctu13_spider_owl_neurips2024.py --seed="+str(seed_value)+" --cos_dist="+str(args.cos_dist)+" --mode_val="+str(args.mode_val)+" --ds="+str(args.ds)+" --lr="+str(args.lr)+" --wd="+str(args.wd)+" --alpha="+str(args.alpha)+" --gpu="+str(args.gpu)+" --filename="+str(temp_file_name)+" --bool_gpm="+str(args.bool_gpm)+" --mem_strat="+str(args.mem_strat)+" --b_m="+str(args.b_m)+" --label_ratio="+str(args.label_ratio)+" --lab_samp_in_mem_ratio="+str(args.lab_samp_in_mem_ratio)+" --nps="+str(args.nps)+" --bma="+str(args.bma)+" --training_cutoff="+str(args.training_cutoff)+" --bool_closs="+str(args.bool_closs)+" --mlps="+str(args.mlps)],shell=True,cwd=curr_dir)
        proc.communicate()
