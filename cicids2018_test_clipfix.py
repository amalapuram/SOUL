    ...
    for epoch in range(epochs):
        ...
        # for epoch in range(epochs):
            ...
            if task_id > 0:
                ...
                # TASK0 OUTPUT ANCHOR loss (LwF-style): penalize the current model's
                # predictions on a random mini-batch of task 0's own validation data
                # for drifting away from what task 0's just-trained (frozen) model
                # predicted on the same samples. Only active from task 1 onward.
                if TASK0_ANCHOR_X is not None:
                    ...
                    if np.random.rand() < 0.002:
                        print(f"[ANCHOR-DIAG] task={task_id} t0_anchor_loss={t0_anchor_loss.item():.6f} total_loss={total_loss.item():.6f}")
                if bool_gpm:
                    total_loss.backward()
                    bn_counter = 0
                    for k, (m,params) in enumerate(model.named_parameters()):
            ...
            # GRADIENT CLIPPING (standard technique, applied uniformly, not
            # dataset-specific): bounds the global gradient norm right before
            # the optimizer step, after GPM projection. Guards against any
            # single loss term (e.g. the anchor loss) producing a
            # destabilizing gradient spike, without changing what is being
            # optimized. max_norm=5.0 is a standard, unremarkable default.
            grad_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=5.0)
            if np.random.rand() < 0.002:
                print(f"[GRADNORM-DIAG] task={task_id} pre_clip_grad_norm={grad_norm.item():.4f}")
