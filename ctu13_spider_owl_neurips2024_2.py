...
# calibration-update branches below can ever fire. See cicids2017's memory
# notes for the full log-evidence trace of this bug's effect.
...
def taskwise_lazytrain():
    ...
    with torch.no_grad():
        for vt in range(len(task_order)):
            ...
            # prior-mean): see cicids2018_spider_owl_2.py for the full trace.
            # Pure prior-strength formula, trust = n_pos / (n_pos + K), no
            # separate fixed ceiling multiplier -- an earlier version capped
            # this at 0.6 regardless of sample size, which (once the softmax
            # scale bug elsewhere in this function was fixed) was needlessly
            # holding well-sampled tasks back from their genuinely-optimal,
            # more conservative threshold. Shrink only protects against a
            # low-n noisy pick, so it is now driven entirely by n_pos.
