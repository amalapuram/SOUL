...
from utils.utils import clustering_class_imbalance
from utils.vime_utils_pytorch import mask_generator, pretext_generator
from utils.vime_self_pytorch import vime_self

# from utils.vime_semi_pytorch import vime_semi
...
# BUGFIX (traced via [THRESH-CALIB]-style instrumented logging, not inferred):
# these must start as None, not 0. The calibration-update logic in
# train_and_gradient() is written around an `if X is None: ... else: ...`
# structure -- clearly intended as a "no calibration measurement yet" sentinel
# -- but the literal initial value here was the float 0, so `is None` never
# fires and the code always takes the "already calibrated" EMA branch. That
# branch explicitly skips updating on an exact-zero measurement (a reasonable
# guard against one noisy small-sample draw overriding real accumulated
# evidence) -- but with the prior ALSO already 0, a single zero draw (observed
# directly: task 1's own dry-run calibration measured 0/60 true positives for
# the attack/minority class -- a real, non-degenerate sample, not empty data)
# leaves the estimate frozen at exactly 0 for the rest of the run, forever.
# This silently zeroes out self-labeled-attack selection for every later
# unseen task regardless of that task's own candidate quality -- confirmed via
# [SELF-LABEL-ACC] logging showing unseen tasks with 98-99% ground-truth-
# verified candidate pools (thousands of samples) still selecting exactly 0.
truth_agreement_fraction_0, truth_agreement_fraction_1 = None, None

# SELF-LABEL ACCURACY tracking: one entry per unseen (OWL self-labeled) task,
# appended inside owl_data_labeling_strategy() when unseen_task=True -- i.e.
# measured on the ACTUAL final self-labeled samples used for training that
# task, not on the pre-selection candidate pool. Ground truth (`y`) is only
# used here for measurement/reporting -- never fed back into training.
...
def compute_distill_loss_self_supervision(p_m, K,unlabeled_x,encoder_model):
    ...
    yu_hat_list = []
    for _ in range(K):
        m_batch = mask_generator(p_m, unlabeled_x)
        temp_umlabeld_X = unlabeled_x.clone().to('cpu')
        _, xu_batch_temp = pretext_generator(m_batch,temp_umlabeld_X)
        xu_batch_temp = encoder_model(xu_batch_temp.to(device))
        # print("hey size",xu_batch_temp.shape)
        class_probs = [] 
        with torch.no_grad():
            for model in models:
                # outputs = torch.softmax(model(xu_batch_temp), dim=1)
                outputs = model(xu_batch_temp)
                class_probs.append(outputs)
        # xu_batch_list.append(xu_batch_temp)
        avg_probs = torch.stack(class_probs).mean(dim=0)
        yu_hat_list.append(avg_probs)
    # xu_batch = torch.stack(xu_batch_list, dim=1)
    yu_hat_stacked = torch.stack(yu_hat_list, dim=1) 

    loss_unsupervised = torch.mean(torch.var(yu_hat_stacked, 1))
...
# )
    ...
    # TRUST FEEDBACK LOOP: see cicids2018_spider_owl_2.py for the full trace.
    # Blend this task's ACTUALLY MEASURED self-label accuracy into
    # truth_agreement_fraction_0/1, on top of the seen-task agreement proxy
    # already blended in elsewhere -- grounds future trust in realized
    # outcomes instead of a proxy that can silently fail to transfer. Same
    # beta-EMA blend as the seen-task update, applied only when this task had
    # samples of that class to measure.
    # Confidence-weight the measurement's influence on the EMA by its own
    # sample size -- see cicids2018_spider_owl_2.py for the full trace (an
    # unweighted blend let a single-sample lucky measurement whipsaw trust
    # and tank aggregate self-label accuracy). Same K=30 prior-strength
    # constant already used for the threshold-calibration fix.
    TRUST_MEASUREMENT_PRIOR_STRENGTH = 30
    if class0_total > 0:
        if truth_agreement_fraction_0 is None:
            truth_agreement_fraction_0 = c0_acc
        else:
            conf0 = class0_total / (class0_total + TRUST_MEASUREMENT_PRIOR_STRENGTH)
            truth_agreement_fraction_0 = ((1 - conf0 * (1 - beta)) * truth_agreement_fraction_0
                                           + conf0 * (1 - beta) * c0_acc)
    if class1_total > 0:
        if truth_agreement_fraction_1 is None:
            truth_agreement_fraction_1 = c1_acc
        else:
            conf1 = class1_total / (class1_total + TRUST_MEASUREMENT_PRIOR_STRENGTH)
            truth_agreement_fraction_1 = ((1 - conf1 * (1 - beta)) * truth_agreement_fraction_1
                                           + conf1 * (1 - beta) * c1_acc)
...
TASK_CALIBRATED_THRESHOLD = {}   # task_id -> classification threshold chosen from that
                                  ...
                                  # PR-AUC (ranking) has repeatedly looked fine even when
                                  # FNR at a fixed 0.5 cutoff was terrible -- a threshold-
                                  # calibration signature, not a ranking-quality one.

# TASK0 OUTPUT ANCHORING: task 0's own post-training validation PR-AUC is ~0.998-0.999
# in EVERY seed (verified directly from logs -- essentially zero seed variance), yet its
# FINAL test-set PR-AUC after all later tasks train swings from ~0.53 to ~0.99 by seed.
# That proves the instability is entirely injected by later tasks' training, not by task
# 0's own optimization. GPM (weight protection), replay composition, and checkpoint
# selection have all been tried to address this; this is a different, more direct lever:
# explicitly penalize the CURRENT model's predictions on task 0's own validation data from
# drifting away from what task 0's just-trained (frozen) model predicted, added as an
# extra loss term during every later task's training -- a Learning-without-Forgetting-style
# output-space anchor, distinct from GPM's weight-space protection.
TASK0_ANCHOR_X = None
...
def train_and_gradient(str_train_model,tasks,task_class_ids,task_id,feature_list,threshold,X_val,y_val,bool_reorganize_memory,owl_data_labeling=False):
    ...
    # Gradient-bias diagnostic buffers/analysis are disabled -- see the
    # commented-out block below. This function is not on the active training
    # path (taskwise_lazytrain() calls train(), not this function); kept for
    # reference only.
    ...
    for epoch in range(epochs):
        ...
        # for epoch in range(epochs):
            ...
            # Gradient-bias diagnostic (benign-vs-attack gradient norm/cosine
            # measurement, fed only into now-disabled plotting) is disabled --
            # this function is not on the active training path. Reference only:
            # benign_mask = (lab_y == 0)
            # attack_mask = (lab_y == 1)
            # loss_b_unsup, loss_a_unsup = 0, 0
            # if task_id > 0 and str_train_model != "student_supervised":
            ...
            #     loss_b_unsup = loss_fn(masked_unsup_x[benign_unsup_mask], F.one_hot(lab_unsup[benign_unsup_mask], num_classes=2).float())
            #     loss_a_unsup = loss_fn(masked_unsup_x[attack_unsup_mask], F.one_hot(lab_unsup[attack_unsup_mask], num_classes=2).float())
            # if benign_mask.any() and attack_mask.any():
            #     opt.zero_grad()
            #     loss_b = loss_fn(y_pred[benign_mask], F.one_hot(lab_y[benign_mask], num_classes=2).float()) + loss_b_unsup
            #     loss_b.backward(retain_graph=True)
            #     g_b = grad_vector_all_layers(model)
            #     opt.zero_grad()
            #     loss_a = loss_fn(y_pred[attack_mask], F.one_hot(lab_y[attack_mask], num_classes=2).float()) + loss_a_unsup
            #     loss_a.backward(retain_graph=True)
            #     g_a = grad_vector_all_layers(model)
            #     # norm/cosine bookkeeping into grad_ratio_list etc. omitted -- fed only disabled plotting.
            ...
            if task_id > 0:
                ...
                # TASK0 output-anchor loss lives in train() now -- this function
                # is not on the active training path.
                # print(y_pred)
        ...
        # Maximin checkpoint selection (Formula 1) lives in train() now -- this
        # function is not on the active training path. Reverted to the original
        # pooled-validation checkpoint metric for reference.
        val_pred,val_gt = [],[]
    ...
    # TASK0 output anchoring and threshold calibration both moved to train()
    # and taskwise_lazytrain() respectively -- this function is not on the
    # active training path (see module-level note near its definition).
    ...
    # Gradient-proof plotting was already disabled (if False) before this
    # function stopped being called; omitted entirely now. See git history /
    # the original cicids2017_spider_owl_neurips2024.py for the full block.
    return feature_list
...
def train(str_train_model,tasks,task_class_ids,task_id,feature_list,threshold,X_val,y_val,bool_reorganize_memory,owl_data_labeling=False):
    ...
    for epoch in range(epochs):
        ...
        # for epoch in range(epochs):
            ...
            if task_id > 0:
                ...
                if bool_gpm:
                    ...
                    #
                    # MAIN-GRADIENT clip (separate safety net; see
                    # cicids2018_spider_owl_2.py for the full trace): guards
                    # against the sup_loss/distil_loss gradient itself
                    # occasionally exploding on a bad batch, independent of and
                    # before the anchor logic below, at the same max_norm=5.0
                    # already confirmed to sit well above any normal gradient.
        ...
        # (INCLUDING current) on its OWN validation slice separately (PR-AUC),
        # use the MINIMUM per-task PR-AUC as the EarlyStopping criterion --
        # not a pooled/concatenated metric, which can hide a regression on an
        # early task behind good performance on a larger, more recent task.
        ...
        # has a task with ~91 total minority-class samples, and a
        # composite score including FPR/FNR there produced catastrophically
        # noisy checkpoint decisions via the min() over tasks. FNR itself is
    ...
    # Threshold calibration runs once, after ALL training completes -- see
    # taskwise_lazytrain(), right before the final testing() calls.
...
def taskwise_lazytrain():
    ...
    # BUGFIX companion to the None-sentinel fix above: truth_agreement_fraction_1
    # can legitimately still be None here (no seen task ever produced a usable
    # nonzero measurement). min(None, 0.1) would raise TypeError -- but more
    # importantly, owl_data_labeling_strategy() ALREADY has its own explicit
    # fallback for exactly this "no calibration data yet" case, at the point of
    # actual use: `if truth_agreement_fraction_1 is None ...: truth_agreement_fraction_1 = 1`
    # (full trust, since there's no evidence yet to distrust the mechanism).
    # That fallback could never fire before this fix, because the value was
    # never actually None (init bug above). We preserve None here rather than
    # inventing our own default, so the code's own pre-existing intended
    # fallback finally executes as originally written.
    ...
    # THRESHOLD CALIBRATION (once, at the very end, from the truly final model
    # state -- see train_and_gradient() for the two per-task variants tried and
    # rejected before this one). PR-AUC has repeatedly looked fine even when FNR
    # at a fixed 0.5 cutoff was terrible; checkpoint-selection fixes don't address
    # that. For every task 0..len(task_order)-1, pick the threshold maximizing
    # Youden's J = TPR - FPR on that task's own validation set, using this final
    # post-training model -- validation set only, never the test set.
    student_model1.eval()
    with torch.no_grad():
        for vt in range(len(task_order)):
            # Task 0 is EXCLUDED from calibration -- across both shrunk and
            # unshrunk versions, calibration was inconsistent for task 0
            # specifically (sometimes neutral, sometimes actively worse than the
            # default 0.5 -- e.g. one seed's task-0 threshold calibrated to 0.79,
            # well above 0.5, worsening FNR), while every OTHER task benefited
            # consistently at both label ratios. Task 0's own PR-AUC/ranking is
            # already the most fragile, highest-variance metric in this whole
            # investigation; a single threshold shift doesn't reliably capture
            # its false negatives the way it does for the other, better-separated
            # tasks. Leaving it at the default 0.5 is the safer, evidence-based
            # choice specifically for this task.
            if vt == 0:
                continue
            vt_X, vt_y = val_x_all_tasks[vt], val_y_all_tasks[vt]
            vt_loader = torch.utils.data.DataLoader(dataset(vt_X, vt_y),
            ...
            # SHRINKAGE toward 0.5: a single validation-optimal threshold can
            # occasionally generalize badly to the test set (originally
            # observed: 2 of 30 task/seed calibrations collapsed to FNR=1.0 on
            # test despite a sane-looking validation pick -- almost certainly
            # the same softmax-scale bug fixed elsewhere in this function,
            # given it produced exactly this symptom on cicids2018; not
            # re-confirmed here, but the fixed 0.6 blanket damping that
            # followed from it is superseded below regardless).
            #
            # ADAPTIVE SHRINKAGE (empirical-Bayes style, shrink-to-prior-mean):
            # see cicids2018_spider_owl_2.py for the full root-cause trace.
            # Pure prior-strength formula, trust = n_pos / (n_pos + K), no
            # separate fixed ceiling multiplier -- the earlier 0.6 cap was
            # needlessly holding well-sampled tasks back from their
            # genuinely-optimal, more conservative threshold once the softmax
            # scale bug was fixed. Shrink only protects against a low-n noisy
            # pick, so it is now driven entirely by n_pos.
            n_pos = int(np.sum(np.asarray(vt_gt) == 1))
            SHRINK_PRIOR_STRENGTH = 30  # pseudo-samples
...
# ])
    ...
    # Exact ground-truth accuracy of the samples the OWL mechanism actually
    # self-labeled and fed into training for each unseen task (see
    # owl_data_labeling_strategy() -> SELF_LABEL_ACCURACY_LOG). Ground truth is
    # used here for measurement/reporting only, never for label generation.
...
def testing(training_cutoff, seen_data=False):
    ...
    for task_id,task in enumerate(testing_tasks, start = start_id):
        ...
        # Compute FPR and FNR at this task's calibrated threshold (Youden's J,
        # picked from validation data during training -- see TASK_CALIBRATED_THRESHOLD
        # / train_and_gradient), falling back to 0.5 if this task was never trained
        # (shouldn't happen in practice, but keeps testing() safe standalone).
    ...
    # Always print the per-task PR-AUC/FPR/FNR table, even for a single test task
    # (N<2), where an AUT value isn't defined but the per-task numbers still are.
    ...
    print(tabulate(pnt_table, headers=headers, tablefmt='grid'))
...
if __name__ == "__main__":
    ...
    global labels_ratio,no_of_rand_samples,l_rate,w_decay,batch_minority_allocation,b_m,alpha,lab_samp_in_mem_ratio,bool_gpm,mem_strat,temp_filename,auc_result,seed,bool_closs,mlps,training_cutoff, epochs, ds, beta,cos_dist_ip, mode_value
