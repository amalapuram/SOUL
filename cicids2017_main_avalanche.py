...
if __name__ == "__main__":
    ...
    parser.add_argument('--lr', type=float, default=1e-3, metavar='S',help='batch memory ratio(default: 0.2)')
    parser.add_argument('--w_d', type=float, default=1e-3, metavar='S',help='labeled ratio (default: 0.1)')
    ...
    for seed_value in seed_list:
        ...
        proc = subprocess.Popen(["python cicids2017_avalanche.py --seed="+str(seed_value)+" --ds="+str(args.ds)+" --gpu="+str(args.gpu)+" --training_cutoff="+str(args.training_cutoff)+" --filename="+str(temp_file_name)+" --w_d="+str(args.w_d)+' --cl_strat='+str(args.cl_strat)+" --lr="+str(args.lr)],shell=True,cwd=curr_dir)
        proc.communicate()
    ...
    # aut_average = [sum(sub_list) / len(sub_list) for sub_list in zip(*aut_results_values)]
    # print("{:<20}  {:<20}  {:<20}  ".format('Cols','AUT(Benign)','AUT(Attack)'))
    # print("-"*80)
    # print("{:<20}  {:<20}  {:<20} ".format('Average',float(str(aut_average[0])[:5]), float(str(aut_average[1]))))    
    # print("-"*80)
    # total_time = time.time()-start_time
    # print("\ntotal execution time is %.3f seconds" % (total_time))
    # print("avg execution time %.3f seconds"%(total_time/len(seed_list)))