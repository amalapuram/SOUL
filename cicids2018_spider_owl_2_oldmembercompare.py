...
def _gpu_cosine_threshold_calibration(candidates, memory_X_t, memory_y_t, mode_value,
                                       target_label, device, batch_size=20000):
    """GPU/vectorized replacement for the old per-candidate Python loop that
    estimated the adaptive cosine-distance threshold (np.argsort + manual
    5%-chunk counting, one candidate row at a time). Mathematically IDENTICAL
    accept rule -- first 5%-of-memory chunk boundary (by ascending cosine
    distance) at which the target_label's share of neighbors seen so far
    clears mode_value% -- just computed for an entire batch of candidate rows
    at once via torch.sort + cumsum instead of a Python for-loop calling
    np.argsort per row. Returns the same `candidate_thresholds` list the old
    code built (one scalar per candidate that hit the condition).
    """
    m = memory_X_t.shape[0]
    loop_range = max(int(0.05 * m), 1)
    boundaries = list(range(loop_range - 1, m, loop_range))
    if boundaries[-1] != m - 1:
        boundaries.append(m - 1)
    boundaries_t = torch.tensor(boundaries, device=device, dtype=torch.long)

    is_target = (memory_y_t == target_label).float()
    ...
    for start in range(0, candidates.shape[0], batch_size):
        ...
        dist = 1.0 - cand_n @ memory_X_t.T                    # (b, m) -- same cos_dist as cdist(...,'cosine')
        sorted_dist, sorted_idx = torch.sort(dist, dim=1)
        cum_target = torch.cumsum(is_target[sorted_idx], dim=1)
        cum_other = torch.cumsum(is_other[sorted_idx], dim=1)
        b_target = cum_target[:, boundaries_t]
        b_other = cum_other[:, boundaries_t]
        b_total = torch.clamp(b_target + b_other, min=1)
        mode_pct = torch.maximum(b_target, b_other) / b_total * 100.0
        hit = (b_target >= b_other) & (mode_pct >= mode_value)   # (b, n_boundaries)
        any_hit = hit.any(dim=1)
        first_hit_col = torch.argmax(hit.float(), dim=1)
        dist_at_boundary = sorted_dist[:, boundaries_t]
        row_idx = torch.arange(dist_at_boundary.shape[0], device=device)
        picked = dist_at_boundary[row_idx, first_hit_col][any_hit]
        if picked.numel() > 0:
            thresholds.extend(picked.detach().cpu().numpy().tolist())
        del cand_t, cand_n, dist, sorted_dist, sorted_idx, cum_target, cum_other
    return thresholds


def _gpu_cosine_member_inference_select(candidates, memory_X_t, memory_y_t, threshold,
                                         mode_value, flip_label, device, batch_size=20000):
    """GPU/vectorized replacement for the old per-candidate Python loop that
    filtered cos_dist < threshold and called scipy.stats.mode per row. Same
    accept rule: majority label among memory points within `threshold` cosine
    distance, only if that majority clears mode_value%; otherwise (or if zero
    memory points are within threshold) assign flip_label -- identical
    semantics to the old "no confident neighborhood -> reject" fallback.
    """
    is_0 = (memory_y_t == 0).float()
    is_1 = (memory_y_t == 1).float()
    n = candidates.shape[0]
    out_labels = np.empty(n, dtype=np.int64)
    supports = np.empty(n, dtype=np.float64)
    for start in range(0, n, batch_size):
        ...
        support = mask.sum(dim=1)
        count0 = (mask * is_0.unsqueeze(0)).sum(dim=1)
        count1 = (mask * is_1.unsqueeze(0)).sum(dim=1)
        support_safe = torch.clamp(support, min=1)
        maj0 = count0 >= count1
        maj_pct = torch.where(maj0, count0, count1) / support_safe * 100.0
        maj_label = torch.where(maj0, torch.zeros_like(count0), torch.ones_like(count0))
        confident = (support > 0) & (maj_pct > mode_value)
        labels = torch.where(confident, maj_label, torch.full_like(maj_label, float(flip_label)))
        out_labels[start:end] = labels.detach().cpu().numpy().astype(np.int64)
        supports[start:end] = support.detach().cpu().numpy()
        del cand_t, cand_n, dist, mask
    return out_labels, supports
...
def owl_data_labeling_strategy(X, y, y_classname, model, unseen_task=True):
    ...
    # ABLATION BASELINE (cicids2018_spider_owl_2_oldmembercompare.py only):
    # this restores the ORIGINAL exhaustive-cdist, sample-to-sample member
    # inference against the raw buffer-memory points (cos_dist against
    # memory_X directly, no model embedding, no clustering) -- exactly the
    # mechanism cicids2018_spider_owl_2.py's cluster_memory_for_member_inference
    # / cluster_based_member_inference replaced. Everything else in this file
    # (softmax fix, main-grad clip, adaptive shrinkage, metadata.py fix) is
    # UNCHANGED from cicids2018_spider_owl_2.py, and the ground-truth-free
    # "accept everything the gate clears" selection policy below is also kept
    # as-is (NOT reverted to the old truth_agreement_fraction ground-truth
    # volume throttle, which was dropped earlier this session as unrealistic
    # for open-world deployment) -- so this is a clean single-variable swap:
    # clustering vs. raw cdist, holding every other fix constant.
    global adp_attack_cos_dist, adp_benign_cos_dist
    purity_threshold = mode_value  # kept on the old 0-100 percentage scale

    # GPU-batched, vectorized (no per-candidate Python loop) versions of the
    # exact same cosine-distance member inference -- see
    # _gpu_cosine_threshold_calibration / _gpu_cosine_member_inference_select
    # docstrings. Needed because a plain cdist(candidates, memory_X) here is
    # up to ~1.8M x 13,334 = ~197GB (float64) -- too large to materialize at
    # all, on GPU or CPU, hence batching; and profiling the CPU version showed
    # the per-row np.argsort/scipy.stats.mode Python loops cost as much as the
    # distance matrix itself, so both are vectorized, not just cdist.
    memory_X_t = torch.as_tensor(memory_X, dtype=torch.float32, device=device)
    memory_X_t = torch.nn.functional.normalize(memory_X_t, p=2, dim=1, eps=1e-12)
    memory_y_t = torch.as_tensor(np.asarray(memory_y), dtype=torch.float32, device=device)
    ...
    if len(top_class_0_indices) != 0:
        ...
        # Member inference of the top samples based on cosine distance from
        # every raw buffer-memory sample (OLD mechanism, GPU-batched).
        start_inference = time.time()

        if unseen_task == False:
            # ================= Estimate cos_dist_ adaptively for benign =================
            cos_time = time.time()
            candidate_thresholds = _gpu_cosine_threshold_calibration(
                top_class_0_data, memory_X_t, memory_y_t, mode_value, target_label=0, device=device)
            ...
            print(f"[INFO] Adaptive cosine distance threshold (benign) = {adp_benign_cos_dist:.4f}")

        member_inference_class_0, support_0 = _gpu_cosine_member_inference_select(
            top_class_0_data, memory_X_t, memory_y_t, adp_benign_cos_dist, mode_value,
            flip_label=1, device=device)
        nonzero_support = support_0[support_0 > 0]
        if nonzero_support.size > 0:
            print(f"[OLD-MI] class0: avg sample support={nonzero_support.mean():.2f} "
                  f"(+/-{nonzero_support.std():.2f})")
        end_inference = time.time()
    ...
    if len(top_class_1_indices) > 1:#!= 0:
        ...
        # Member inference of the top samples based on cosine distance from
        # every raw buffer-memory sample (OLD mechanism, GPU-batched) -- see
        # class-0 block above for the full ablation rationale.
        start_inference = time.time()

        if unseen_task == False:
            # ================= Estimate cos_dist_ adaptively for attack =================
            cos_time = time.time()
            candidate_thresholds_attack = _gpu_cosine_threshold_calibration(
                top_class_1_data, memory_X_t, memory_y_t, mode_value, target_label=1, device=device)
            ...
            if len(candidate_thresholds_attack) > 0:
                ...
                adp_attack_cos_dist = 0.2

            time_elapsed = time.time() - cos_time
            print(f"[TIMER] Adaptive ATTACK cosine distance estimation: {time_elapsed:.3f}s")
            print(f"[INFO] Adaptive cosine distance threshold (attack) = {adp_attack_cos_dist:.4f}")

        member_inference_class_1, support_1 = _gpu_cosine_member_inference_select(
            top_class_1_data, memory_X_t, memory_y_t, adp_attack_cos_dist, mode_value,
            flip_label=0, device=device)
        nonzero_support = support_1[support_1 > 0]
        if nonzero_support.size > 0:
            print(f"[OLD-MI] class1: avg sample support={nonzero_support.mean():.2f} "
                  f"(+/-{nonzero_support.std():.2f})")
        end_inference = time.time()
...
def train(str_train_model,tasks,task_class_ids,task_id,feature_list,threshold,X_val,y_val,bool_reorganize_memory,owl_data_labeling=False):
    ...
    if owl_data_labeling == False:
        ...
        labeled_X, labeled_y, labeled_y_classname, X_unlab, labeled_indicies,unlabeled_indicies = owl_data_labeling_strategy(X, y, y_classname, model, unseen_task=True)
