...
import faiss
from active_candidate_selection import (
    class_adaptive_mode_value, compute_novelty_score_clustering,
    ensemble_disagreement_score, build_analyst_queue,
    compute_confusion_matrix_binary, estimate_class_prior_bbse,
)
...
CI_list = []
avg_CI = None
bbse_confusion_matrix = None  # updated from each seen task's real labels; None until task 1's calibration call
...
def _embed_in_batches(X_arr, model):
    """Run X_arr through model in eval/no_grad mode and return the
    penultimate-layer representation (model.act['hidden7'], populated by
    every forward pass -- no architecture change needed), batched to bound
    GPU memory. Used by both memory-clustering and candidate-assignment
    below, so both live in the SAME learned embedding space instead of raw
    input features."""
    model.eval()
    loader = torch.utils.data.DataLoader(dataset(X_arr, np.zeros(X_arr.shape[0])),
                                          batch_size=batch_size, num_workers=0)
    chunks = []
    with torch.no_grad():
        for data, _ in loader:
            _ = model(data.to(device))
            chunks.append(model.act['hidden7'].detach().cpu().numpy())
    emb = np.ascontiguousarray(np.concatenate(chunks, axis=0), dtype='float32')
    faiss.normalize_L2(emb)  # unit-normalize: squared-Euclidean k-means on
                              # normalized vectors is equivalent to cosine-
                              # distance clustering (standard trick), keeping
                              # the same distance notion the old cdist(...,
                              # 'cosine') member-inference used.
    return emb


def cluster_memory_for_member_inference(memory_X, memory_y, model,
                                         min_cluster_size=50, max_clusters=25):
    """
    Cluster the buffer memory in the model's own learned embedding space
    (see _embed_in_batches) using faiss's GPU k-means -- replaces exhaustive
    per-candidate cosine distance against every raw memory point (confirmed
    via direct profiling to cost ~63% of total training wall-clock time on
    cicids2018 -- 13,242 of 21,155 seconds in one run) with O(n_clusters)
    comparisons instead of O(memory_size), and moves the comparison itself
    into the space the network already learned to separate classes in,
    instead of raw input features. Returns the trained faiss index plus a
    PURITY score per cluster: the fraction of that cluster's OWN memory
    members (real labels -- that's what a replay buffer is, not a ground-
    truth violation) agreeing with the cluster's majority label. Purity is
    the trust signal member-inference decisions are based on below --
    computed entirely from memory's own already-known labels, no ground
    truth on new (candidate) samples is used anywhere in this mechanism.
    n_clusters scales with how much memory exists for this class (more data
    -> finer sub-clusters, e.g. separating attack sub-types instead of
    lumping them into one blurred centroid), bounded so it neither
    over-fragments small memories nor grows unbounded with huge ones.
    """
    mem_emb = _embed_in_batches(memory_X, model)
    memory_y_np = np.asarray(memory_y)

    n_clusters = int(np.clip(len(memory_y_np) // min_cluster_size, 2, max_clusters))
    d = mem_emb.shape[1]
    km = faiss.Kmeans(d, n_clusters, niter=20, gpu=True, seed=seed)
    km.train(mem_emb)
    mem_dist, cluster_ids = km.index.search(mem_emb, 1)  # mem_dist: squared L2 to nearest centroid
    cluster_ids = cluster_ids.ravel()
    mem_dist = mem_dist.ravel()

    purity = np.zeros(n_clusters)
    majority_label = np.full(n_clusters, -1)
    radius = np.zeros(n_clusters)
    for c in range(n_clusters):
        member_mask = cluster_ids == c
        members = memory_y_np[member_mask]
        if len(members) == 0:
            continue
        vals, counts = np.unique(members, return_counts=True)
        majority_label[c] = vals[np.argmax(counts)]
        purity[c] = counts.max() / len(members)
        # Per-cluster acceptance RADIUS: this cluster's own members' mean
        # distance to its centroid -- a self-calibrating notion of "how
        # spread out this region actually is", used below to reject
        # candidates that are merely closEST to this cluster among the k
        # available (faiss always returns a nearest centroid, however far)
        # without actually resembling anything in it. A tight cluster gets a
        # tight acceptance radius; a diffuse one gets a looser one -- no
        # single global distance cutoff, which would be either too strict
        # for naturally-spread-out regions or too loose for tight ones.
        radius[c] = mem_dist[member_mask].mean()

    return km, purity, majority_label, radius


def cluster_based_member_inference(candidate_X, model, km, purity, majority_label, radius,
                                    purity_threshold, radius_margin=2.0):
    """
    Embed candidates in the SAME space as cluster_memory_for_member_inference
    (same model, same _embed_in_batches call), assign each to its nearest
    memory cluster via faiss, and accept that cluster's majority label only
    if BOTH (a) the cluster's purity clears purity_threshold AND (b) the
    candidate is actually within radius_margin x that cluster's own radius
    -- not just "closest among the k available clusters, however far".
    (b) is necessary: confirmed directly that without it, a large genuinely-
    novel/shifted population (a new attack pattern not resembling anything
    in memory yet) can get vacuumed wholesale into the nearest high-purity
    cluster regardless of true distance, since faiss always returns SOME
    nearest centroid -- purity alone can't tell "close and pure" apart from
    "merely least-far and pure", and only the former is trustworthy. This
    mirrors the old mode_value + adaptive-cosine-distance-threshold
    accept/reject semantics (purity_threshold = mode_value/100, same CLI
    knob) but the trust decision is now per-region (per cluster) instead of
    one global scalar (truth_agreement_fraction) shared across an entire
    class -- directly addresses why attack self-labeling was so much worse
    than benign: a single global number was blurring together attack
    sub-types with very different actual reliability.
    """
    cand_emb = _embed_in_batches(candidate_X, model)
    cand_dist, assigned_cluster = km.index.search(cand_emb, 1)
    assigned_cluster = assigned_cluster.ravel()
    cand_dist = cand_dist.ravel()
    cluster_purity_per_sample = purity[assigned_cluster]
    cluster_radius_per_sample = radius[assigned_cluster]
    predicted_label = majority_label[assigned_cluster]
    accept_mask = (cluster_purity_per_sample >= purity_threshold) & \
                  (cand_dist <= radius_margin * cluster_radius_per_sample)
    return predicted_label, accept_mask


def owl_data_labeling_strategy(X, y, y_classname, model, unseen_task=True):

    global owl_self_labelled_count_class_0, owl_self_labelled_count_class_1
    global owl_analyst_labelled_count_class_0, owl_analyst_labelled_count_class_1
    global truth_agreement_fraction_0,truth_agreement_fraction_1
    global avg_CI
    global bbse_confusion_matrix
    ...
    if unseen_task == False:
        # Update the BBSE confusion matrix using this task's real labels --
        # legitimate here (seen/supervised task, same ground-truth access
        # the existing avg_CI/CI_list computation in train() already uses).
        bbse_confusion_matrix = compute_confusion_matrix_binary(y, predicted_labels.detach().cpu().numpy())
    ...
    X_unlab = None
    unlabeled_indicies, labeled_indicies = np.array([]), np.array([])
    # RCA FIX (see CICIDS2018_CDIST_VS_CLUSTERING_ABLATION.md): the CReST
    # relaxed threshold_1 was found to let large volumes of WRONGLY self-
    # labeled attack samples straight into the persistent replay buffer --
    # e.g. task 6/ratio=0.4: 78,687 accepted at 0.108% actual accuracy, all
    # written into memory as confirmed "attack". Because member-inference's
    # own trust signal (cluster purity) is computed FROM memory's own
    # labels, this corrupts purity for ALL future tasks and cascades into
    # collapsing accept rates for BOTH classes (observed: task7 benign
    # agreements fell to 0/1,687,222, memory purity mean 0.90->0.78).
    # Fix: relaxed threshold_1 is used for THIS TASK's training signal only
    # (labeled_X/labeled_y below, unchanged) -- memory_safe_mask tracks a
    # PARALLEL, stricter eligibility (original unrelaxed purity_threshold
    # for self-labels, always-safe for analyst-labels since human-verified)
    # that train() uses to decide what's actually allowed into the
    # persistent buffer, protecting long-term memory integrity from a
    # single bad task's mistakes while still getting the relaxed
    # threshold's training-signal benefit immediately.
    memory_safe_mask = np.array([], dtype=bool)
    member_inference_class_0 = 0 # dummy assignment
    member_inference_class_1 = 1 # dummy assignment
    selection_count_0, selection_count_1 = 0, 0
    n_agreements_0, n_agreements_1 = 0, 0
    ...
    # CLUSTER-BASED member inference (replaces exhaustive cdist against every
    # raw memory point): cluster memory ONCE per call, in the model's own
    # embedding space, shared by both the class-0 and class-1 blocks below.
    # See cluster_memory_for_member_inference()'s docstring for the full
    # rationale/profiling evidence. purity_threshold reuses mode_value on the
    # same 0-100 scale as the old accept/reject cutoff -- same CLI knob, not
    # a new hyperparameter.
    purity_threshold = mode_value / 100.0
    mem_km, mem_purity, mem_majority_label, mem_radius = cluster_memory_for_member_inference(memory_X, memory_y, model)
    print(f"[CLUSTER-MI] {len(mem_purity)} memory clusters, purity mean={mem_purity.mean():.4f} "
          f"min={mem_purity.min():.4f} max={mem_purity.max():.4f}")

    # CReST-style (Wei et al., CVPR 2021) class-adaptive purity threshold for
    # the attack class only: memory's own class-1 share is almost always
    # tiny (severe imbalance), and a single shared mode_value threshold was
    # shown this session to starve attack self-labeling near-zero across
    # every seed/ratio tested (see CICIDS2018_CDIST_VS_CLUSTERING_ABLATION.md).
    # Benign (class 0) keeps the original threshold unchanged -- it was
    # already working (~99.9% self-label accuracy), no reason to touch it.
    memory_y_arr = np.asarray(memory_y)
    n_attack_mem = int(np.sum(memory_y_arr == 1))
    n_total_mem = len(memory_y_arr)
    purity_threshold_1 = class_adaptive_mode_value(mode_value, n_attack_mem, n_total_mem) / 100.0
    print(f"[CReST] attack class share in memory: {n_attack_mem}/{n_total_mem} "
          f"({100.0*n_attack_mem/max(n_total_mem,1):.3f}%) -> purity_threshold_1={purity_threshold_1:.4f} "
          f"(base mode_value/100={purity_threshold:.4f})")
    ...
    if len(top_class_0_indices) != 0:
        ...
        # Member inference of the top samples via nearest-cluster purity
        # (see cluster_based_member_inference docstring).
        start_inference = time.time()
        predicted_label_0, accept_mask_0 = cluster_based_member_inference(
            top_class_0_data, model, mem_km, mem_purity, mem_majority_label, mem_radius, purity_threshold)
        member_inference_class_0 = np.where(accept_mask_0, predicted_label_0, 1)
        # ^ candidates whose nearest cluster doesn't clear purity_threshold
        # are given the flipped (non-class-0) label, same semantics as the
        # old "no confident neighborhood found -> reject" fallback.
        end_inference = time.time()
        ...
        n_agreements_0 = np.sum(member_inference_class_0 == 0)
        curr_truth_agreement_fraction_0 = np.sum(top_class_0_truth[member_inference_class_0 == 0] == 0)/np.sum(member_inference_class_0 == 0) if n_agreements_0 > 0 else 0
        if math.isnan(curr_truth_agreement_fraction_0):
            curr_truth_agreement_fraction_0 = 0
        ...
        if unseen_task:
            if n_agreements_0 > 0:
                # SELECTION: accept every candidate the cluster-purity gate
                # already cleared -- no additional truth_agreement_fraction
                # volume throttle. Cluster purity IS the trust/quality gate
                # now (per-cluster, computed from memory's own labels), so a
                # second global-scalar downsampling step on top would be
                # redundant -- and that global scalar (fed by a seen-task
                # proxy that badly overestimated unseen-task reliability on
                # cicids2018, see memory notes) is exactly the mechanism
                # this redesign replaces.
                selection_count_0 = int(n_agreements_0)
                selected_0_indices = top_class_0_indices[member_inference_class_0 == 0]
                ...
                labeled_indicies = np.hstack((labeled_indicies, selected_0_indices))
                memory_safe_mask = np.hstack((memory_safe_mask, np.ones(selection_count_0, dtype=bool)))
                # ^ benign uses the original, unchanged threshold -- always memory-safe.
    ...
    if len(top_class_1_indices) > 1:#!= 0:
        ...
        # Member inference of the top samples via nearest-cluster purity
        # (mem_km/mem_purity/mem_majority_label built once, shared with the
        # class-0 block above -- see cluster_based_member_inference).
        start_inference = time.time()
        predicted_label_1, accept_mask_1 = cluster_based_member_inference(
            top_class_1_data, model, mem_km, mem_purity, mem_majority_label, mem_radius, purity_threshold_1)
        member_inference_class_1 = np.where(accept_mask_1, predicted_label_1, 0)
        # RCA FIX: parallel check against the ORIGINAL unrelaxed threshold,
        # for memory-eligibility only (see memory_safe_mask docstring at the
        # top of this function). Cheap -- reuses the same km/purity/radius,
        # no re-clustering.
        _, accept_mask_1_strict = cluster_based_member_inference(
            top_class_1_data, model, mem_km, mem_purity, mem_majority_label, mem_radius, purity_threshold)
        # ^ candidates whose nearest cluster doesn't clear purity_threshold
        # are given the flipped (non-class-1) label, same semantics as the
        # old "no confident neighborhood found -> reject" fallback.
        end_inference = time.time()
        ...
        n_agreements_1 = np.sum(member_inference_class_1 == 1)
        curr_truth_agreement_fraction_1 = np.sum(top_class_1_truth[member_inference_class_1 == 1] == 1)/np.sum(member_inference_class_1 == 1) if n_agreements_1 > 0 else 0
        if math.isnan(curr_truth_agreement_fraction_1):
            curr_truth_agreement_fraction_1 = 0
        ...
        if unseen_task:
            if n_agreements_1 > 0:
                # SELECTION: accept every candidate the cluster-purity gate
                # already cleared -- see the class-0 block above for the full
                # rationale (no more truth_agreement_fraction volume throttle
                # on top of the purity gate).
                selection_count_1 = int(n_agreements_1)
                selected_1_indices = top_class_1_indices[member_inference_class_1 == 1]
                ...
                labeled_indicies = np.hstack((labeled_indicies, selected_1_indices))
                # RCA FIX: memory-safe only for the subset that ALSO clears
                # the strict, unrelaxed threshold -- the relaxed threshold's
                # extra volume is used for this task's training (below,
                # unchanged) but not trusted into the persistent buffer.
                strict_safe_for_selected = accept_mask_1_strict[member_inference_class_1 == 1]
                memory_safe_mask = np.hstack((memory_safe_mask, strict_safe_for_selected))
    ...
    # NOTE: no ground-truth feedback into truth_agreement_fraction here (a
    # prior version of this code did this, then a second version fixed a
    # low-n whipsaw bug in it -- both dropped entirely: checking self-labeled
    # samples against hidden ground truth is not something a genuine
    # open-world deployment has access to, caught during review of that
    # design). truth_agreement_fraction is no longer used for selection at
    # all -- CLUSTER PURITY (see cluster_memory_for_member_inference /
    # cluster_based_member_inference above) is the trust signal now, and it
    # needs no ground truth of any kind: purity comes entirely from memory's
    # own already-known labels, which a real deployment does have (that's
    ...
    # ATTACK analyst selection: OOD/novelty + ensemble-disagreement ranked,
    # BADGE-diversified (active_candidate_selection.py) instead of uniform
    # random -- targets the analyst's limited budget at the candidates most
    # likely to be genuinely informative (far from anything in memory, or
    # where the model ensemble disagrees), rather than a random slice of the
    # leftover pool. Benign (class 0) is left as random above: it was never
    # the problem (self-label accuracy already ~99.9%), so untargeted
    # sampling there is fine and keeps this change minimal/focused.
    budget_1 = min(len(remaining_1_indices), count_class_1)
    if budget_1 > 0:
        candidates_1 = X[remaining_1_indices]
        novelty_1 = compute_novelty_score_clustering(candidates_1, model, _embed_in_batches, mem_km, mem_radius)
        disagreement_1 = ensemble_disagreement_score(models, candidates_1, device)
        local_idx_1 = build_analyst_queue(candidates_1, novelty_1, disagreement_1, budget_1)
        selected_1_indices = remaining_1_indices[local_idx_1]
    else:
        selected_1_indices = np.array([], dtype=np.int64)

    temp_X = np.vstack((X[selected_0_indices], X[selected_1_indices]))
    # NOTE: uses the ACTUAL selected lengths, not the requested
    # count_class_0/count_class_1 -- the original code used the requested
    # counts here, which silently mismatches temp_X's real row count
    # whenever a remaining pool is smaller than requested (pre-existing,
    # left alone elsewhere; fixed here since this block is being touched
    # anyway and the new selection path makes a short return more likely).
    temp_y = np.hstack(([0]*len(selected_0_indices), [1]*len(selected_1_indices)))
    temp_y_classname = np.hstack(([benign_y_name]*len(selected_0_indices), [attack_y_name]*len(selected_1_indices)))
    print(f'No. of security analyst-labelled samples: {temp_X.shape[0]} (0:{len(selected_0_indices)}, 1:{len(selected_1_indices)})')
    ...
    labeled_y_classname = np.hstack((labeled_y_classname, temp_y_classname)) if labeled_y_classname is not None else temp_y_classname
    labeled_indicies = np.hstack((labeled_indicies, np.hstack((selected_0_indices, selected_1_indices))))
    # Analyst-labeled samples are always memory-safe (human-verified ground
    # truth, not a member-inference guess of any kind).
    memory_safe_mask = np.hstack((memory_safe_mask, np.ones(len(selected_0_indices) + len(selected_1_indices), dtype=bool)))
    print(f'Total no. of labelled samples: {labeled_X.shape[0]}')
    ...
    return labeled_X,labeled_y,labeled_y_classname, X_unlab, labeled_indicies,unlabeled_indicies, memory_safe_mask
...
TASK0_ANCHOR_WEIGHT = 1.0   # relative weight of this loss term vs. sup_loss/distil_loss
...
def train(str_train_model,tasks,task_class_ids,task_id,feature_list,threshold,X_val,y_val,bool_reorganize_memory,owl_data_labeling=False):
    ...
    if owl_data_labeling == False:
        ...
        labeled_X,labeled_y,labeled_y_classname = X[labeled_indicies],y[labeled_indicies],y_classname[labeled_indicies]
        X_unlab,y_unlab,y_unlabclassname = X[unlabeled_indicies],y[unlabeled_indicies],y_classname[unlabeled_indicies]
        # Seen task: labeled_X is real ground truth (a true-labeled subset),
        # not a member-inference guess -- always memory-safe.
        memory_safe_mask = np.ones(len(labeled_indicies), dtype=bool)
        ...
        # Computing class imbalance ratio for the task (0:1)
        if task_id > 0:
            task_truth_agreement_fractions = owl_data_labeling_strategy(X, y, y_classname, model, unseen_task=False)
            print(f'\nCurrent task truth agreement fractions = {task_truth_agreement_fractions}')
            print(truth_agreement_fraction_0, truth_agreement_fraction_1)
            ## accumulation
            if truth_agreement_fraction_0 is None:
        ...
        labeled_X, labeled_y, labeled_y_classname, X_unlab, labeled_indicies,unlabeled_indicies, memory_safe_mask = owl_data_labeling_strategy(X, y, y_classname, model, unseen_task=True)
    ...
    ###Buffer memory organization
    # RCA FIX: only memory_safe_mask-eligible samples go into the persistent
    # replay buffer -- see memory_safe_mask's docstring in
    # owl_data_labeling_strategy(). Training (labeled_X/labeled_y, used
    # elsewhere in this function) still uses the FULL relaxed-threshold set
    # unchanged; this only restricts what gets written into memory_X/memory_y,
    # protecting future tasks' member-inference purity from this task's
    # mistakes.
    memory_safe_mask = np.asarray(memory_safe_mask, dtype=bool)
    print(f'[RCA-FIX] memory-eligible: {memory_safe_mask.sum()}/{len(memory_safe_mask)} of this task\'s labeled samples '
          f'({len(memory_safe_mask) - memory_safe_mask.sum()} relaxed-threshold-only samples excluded from memory)')
        ...
        # for epoch in range(epochs):
            ...
            if task_id > 0:
                ...
                if bool_gpm:
                    ...
                    #
                    # MAIN-GRADIENT clip (separate safety net, added after
                    # observing a genuine loss explosion on a later task --
                    # total_loss climbing from ~30 to 460 within one epoch,
                    # collapsing every task's validation score at once): this
                    # is a distinct failure mode from the anchor-driven one
                    # above -- it is the sup_loss/distil_loss gradient itself
                    # (alpha-weighted distillation term included) occasionally
                    # exploding on a bad batch, with nothing bounding it before
                    # opt.step(). Clipped on its OWN, independent of and before
                    # the anchor logic below, at the same max_norm=5.0 already
                    # confirmed (via logged per-batch norms) to sit well above
                    # any normal/healthy gradient -- so this only intervenes on
                    # genuinely pathological batches and is a no-op otherwise,
                    # unlike the earlier COMBINED main+anchor clip that diluted
                    # normal training and regressed cicids2017.
...
def taskwise_lazytrain():
    ...
    with torch.no_grad():
        for vt in range(len(task_order)):
            ...
            vt_pred, vt_gt = [], []
            for data, target in vt_loader:
                pred = torch.softmax(student_model1(data.to(device)), dim=1)[:, 1].reshape(target.shape)
                vt_pred.extend(pred.detach().cpu().numpy().tolist())
                vt_gt.extend(target.detach().cpu().numpy().tolist())
            ...
            # ADAPTIVE SHRINKAGE toward 0.5 (empirical-Bayes style, shrink-to-
            # prior-mean): a pure prior-strength formula, trust = n_pos /
            # (n_pos + K), with NO separate fixed ceiling multiplier. An
            # earlier version of this line applied a fixed 0.6 ceiling on top
            # (inherited from a pre-softmax-fix version of this file, when the
            # raw threshold was frequently a meaningless, scale-broken value
            # and heavy blanket damping was the only available defense).  With
            # the softmax scale bug fixed elsewhere in this function, the raw
            # Youden's-J threshold is now a properly-scaled probability and,
            # for a well-sampled task (large n_pos), a reliable one -- capping
            # trust at 0.6 regardless of sample size was leaving good
            # separation on the table: confirmed directly on cicids2018
            # seed=1/2, where several tasks had excellent ranking (PR-AUC
            # 0.94-0.99) yet the 0.6-capped threshold still produced FPR
            # 0.19-0.38, because Youden's J's own recommended cut point
            # (raw_thresh, often ~0.8) was damped halfway back to 0.5 even
            # though n_pos there is in the tens of thousands (no sampling-
            # noise justification for holding back). The only reason to
            # shrink at all is protecting against a low-n noisy pick, so the
            # shrink factor is now driven entirely by n_pos vs the prior
            # strength K -- trust -> 1 (raw threshold used as-is) as n_pos
            # grows, trust -> 0 (stay at neutral 0.5) as n_pos shrinks toward
            # zero, with nothing arbitrary left in between.
            n_pos = int(np.sum(np.asarray(vt_gt) == 1))
            SHRINK_PRIOR_STRENGTH = 30  # pseudo-samples; same order of magnitude
                                         # as the smallest observed task sizes
            adaptive_shrinkage = n_pos / (n_pos + SHRINK_PRIOR_STRENGTH)
            TASK_CALIBRATED_THRESHOLD[vt] = 0.5 + adaptive_shrinkage * (raw_thresh - 0.5)
            print(f"[THRESH-CALIB] task={vt} n_pos(val)={n_pos} raw_thresh={raw_thresh:.4f} "
                  f"shrinkage={adaptive_shrinkage:.4f} -> threshold={TASK_CALIBRATED_THRESHOLD[vt]:.4f}")
