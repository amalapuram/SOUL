...
if __name__ == "__main__":
    ...
    for i, lr in enumerate(lr_list):
        for j, wd in enumerate(wd_list):
            for k, batch_size in enumerate(batch_sizes):
                ...
                proc = subprocess.Popen(["python apigraph_spider_owl_copy.py --grid_search=True"+" --ds="+str(args.ds)+" --lr="+str(lr)+" --wd="+str(wd)+" --batch_size="+str(batch_size)+" --alpha="+str(args.alpha)+" --gpu="+str(args.gpu)+" --filename="+str(temp_file_name)+" --bool_gpm="+str(args.bool_gpm)+" --mem_strat="+str(args.mem_strat)+" --b_m="+str(args.b_m)+" --label_ratio="+str(args.label_ratio)+" --lab_samp_in_mem_ratio="+str(args.lab_samp_in_mem_ratio)+" --nps="+str(args.nps)+" --bma="+str(args.bma)+" --training_cutoff="+str(args.training_cutoff)+" --bool_closs="+str(args.bool_closs)+" --mlps="+str(args.mlps)+" --n_epochs="+str(args.n_epochs)],shell=True,cwd=curr_dir)
                proc.communicate()
    ...
    print('For apigraph dataset in owl setting: (only attack shift)')
